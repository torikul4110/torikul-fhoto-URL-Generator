...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Multiple Links to QR - TORIKUL SYSTEM</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
    ...
    <div class="container">
        ...
        <div class="input-area">
            ...
            <div style="margin-top:15px;display:flex;gap:10px;flex-wrap:wrap;">
                ...
                <button class="btn btn-secondary" onclick="clearLinks()">Clear All</button>
            </div>
        </div>
        <div class="result-box" id="resultBox">
            <div class="group-info">
...
<body>
    <div class="container">
        ...
        <div class="group-meta">
            ...
            <div class="btn-group">
                ...
                <button class="btn btn-success" onclick="downloadGroupQR()">⬇️ Download QR</button>
                <button class="btn btn-secondary" onclick="regenerateGroupLink()">🔄 Regenerate</button>
            </div>
            <div style="margin-top:15px;">
        ...
        <div class="gallery-grid">
            ...
            <div class="gallery-item" data-filename="{{ img.filename }}">
                ...
                <div style="display:flex;gap:8px;justify-content:center;margin-top:5px;">
                    <button class="btn btn-primary" style="padding:4px 10px;font-size:0.7em;" onclick="event.stopPropagation();copyToClipboard('{{ img.url }}')">📋</button>
                    <button class="btn btn-success" style="padding:4px 10px;font-size:0.7em;" onclick="event.stopPropagation();downloadImageQR('{{ img.filename }}')">🧾</button>
                    <button class="btn btn-danger" style="padding:4px 10px;font-size:0.7em;" onclick="event.stopPropagation();deleteImageFromGroup('{{ group.id }}','{{ img.filename }}')">🗑️</button>
                </div>
            </div>
            {% endfor %}
        </div>
    ...
    <script>
        ...
        function deleteImageFromGroup(groupId, filename) {
            if (!confirm('Are you sure?')) return;
            fetch('/api/delete-group-image/' + groupId + '/' + filename, { method: 'DELETE' })
                .then(res => res.json())
                .then(data => {
                    if (data.success) {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .link-card .qr-small { text-align: center; padding: 10px; background: #fff; border-radius: 8px; margin-top: 10px; }
        .link-card .qr-small img { max-width: 120px; }
        .btn-group-small { display: flex; gap: 5px; margin-top: 10px; flex-wrap: wrap; }
        .btn-small {
            padding: 4px 10px; border: none; border-radius: 6px;
            font-size: 0.7em; cursor: pointer; transition: all 0.3s;
            color: #fff;
        }
        .btn-small-primary { background: linear-gradient(135deg, #667eea, #764ba2); }
        .btn-small-success { background: linear-gradient(135deg, #51cf66, #40c057); }
    ...
    <div class="container">
        ...
        <div class="links-grid">
            ...
            <div class="link-card" data-linkid="{{ link.link_id }}">
                ...
                <div class="btn-group-small">
                    <button class="btn-small btn-small-primary" onclick="copyToClipboard('{{ link.url }}')">📋 Copy</button>
                    <button class="btn-small btn-small-success" onclick="downloadLinkQR('{{ link.link_id }}')">⬇️ QR</button>
                    <button class="btn-small btn-small-danger" onclick="deleteLink('{{ link.link_id }}')">🗑️ Delete</button>
                </div>
            </div>
...
<head>
    ...
    <title>Group Gallery - TORIKUL</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        html, body {
            width: 100%;
        ...
        .header {
            ...
            padding: 10px 0 20px; border-bottom: 1px solid rgba(255,255,255,0.05);
            margin-bottom: 25px;
        }
        .header h1 {
        ...
        .lightbox .close-btn {
            ...
            font-size: 2.2em; color: #fff; cursor: pointer;
        ...
        .lightbox .nav-btn {
            ...
            color: #fff; font-size: 2.5em; padding: 15px 10px;
        ...
        .lightbox .image-wrapper {
            display: flex; justify-content: center; align-items: center;
            width: 100%; height: 100%; padding: 20px 60px;
        }
        ...
        .lightbox .dots {
            ...
            background: rgba(0,0,0,0.5); padding: 8px 14px; border-radius: 30px;
        ...
        .lightbox .counter {
            ...
            color: rgba(255,255,255,0.5); font-size: 0.95em;
...
<body>
    ...
    <div class="lightbox" id="lightbox">
        ...
        <div class="dots" id="lightboxDots">
            {% for _ in images %}<span class="dot"></span>{% endfor %}
        </div>
    </div>
    ...
    <script>
        (function() {
            ...
            function preload(index) {
                ...
                new Image().src = images[prevIdx].url;
            }
